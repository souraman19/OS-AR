...
def main(config): 
    ...
    if config.MODEL.RESUME:
       start_epoch, max_accuracy = load_checkpoint(config, model.module if hasattr(model, "module") else model, optimizer, lr_scheduler, logger)
    ...
    if config.TEST.ONLY_TEST:
        acc1, all_true, all_pred = validate(val_loader, text_labels, model, config)
        ...
        disp.plot(include_values=True, cmap='Blues', colorbar=True)
        plt.title("Confusion Matrix")
        plt.tight_layout()
        save_path = os.path.join(config.OUTPUT, "confusion_matrix.png")
    ...
    for epoch in range(start_epoch, config.TRAIN.EPOCHS):
        ...
        if get_rank_safe() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
...
if __name__ == '__main__':
    ...
    if world_size > 1:
        ...
        print("⚠️ Single GPU detected — skipping torch.distributed.init_process_group()")
        args.local_rank = 0


    seed = config.SEED + get_rank_safe()
    ...
    # logger
    logger = create_logger(output_dir=config.OUTPUT, dist_rank=get_rank_safe(), name=f"{config.MODEL.ARCH}")
